...
A local web app that aggregates coding-agent chat sessions from **Cursor**, **Grok**, **Codex**, **OpenCode**, **Claude Code**, and **Command Code** into a single timeline sorted by most recent activity.
...
- **Unified inbox** — all chats from six tools in one list
- **Sort by recency** — newest sessions first
- **Search** — filter by title, working directory, tool, or model
- **Source filters** — quick chips for Cursor, Grok, Codex, OpenCode, Claude, Command Code
- **Chat detail view** — click a session to read user/assistant messages
- **Light & dark mode** — theme toggle in the header
- **Chat data is read-only** — never mutates Cursor/Grok/Codex/OpenCode/Claude/Command Code session stores
...
| **Command Code** | `~/.commandcode/projects/**/*.jsonl`                                    | JSONL transcripts + `.meta.json`     |
